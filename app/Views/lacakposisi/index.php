...
<div id="content" class="app-content">
		...
		<script type="text/javascript">
				...
				function save(id = "") {
					...
					$.ajax({
						type: 'POST',
						url: '<?= base_url("lacakposisi/save/") ?>/' + id,
						dataType: 'json',
						data: {
							id_sopir: $('select[name="id_sopir"]').val(),
							posisi_gps: $('input[name="posisi_gps"]').val(),
				...
				$('#toolbar_tambah').on('click', function () {
					$('.main_modal').on('show.bs.modal', function (e) {
						...
						xhr = $.ajax({
							type: 'POST',
							url: '<?= base_url("lacakposisi/tambah/")?>',
							datatype: 'json',
							success: function (data) {
								setTimeout(function () {
				...
				function edit(id) {
					$('.main_modal').on('show.bs.modal', function (e) {
						...
						xhr = $.ajax({
						type: 'POST',
						url: '<?= base_url("lacakposisi/edit/")?>/'+ id,
						dataType: 'json',
						success: function (data) {	
								$('#modal_content').html(data);
			...
			    		keyboard: false
				...
				function remove(id) {
					$.ajax({
						type: 'POST',
						url: '<?= base_url("lacakposisi/delete/") ?>/' + id,
						dataType: 'json',
						success: function (data) {
							var pageno = $('.paginate_active a').data('ci-pagination-page') - 1;
							load_data();
