						...
						<table id="datatableDefault" class="table text-nowrap w-100">
							<thead>
								<tr>
									...
									<th class="border-top-0 pt-0 pb-2">Total Biaya</th>
		...
		<script type="text/javascript">
				...
				$('#toolbar_tambah').on('click', function () {
					$('.main_modal').on('show.bs.modal', function (e) {
						...
						xhr = $.ajax({
							type: 'POST',
							url: '<?= base_url("maintenance/tambah/")?>',
							datatype: 'json',
							success: function (data) {
								setTimeout(function () {
				...
				function edit(id) {
					$('.main_modal').on('show.bs.modal', function (e) {
						...
						xhr = $.ajax({
						type: 'POST',
						url: '<?= base_url("maintenance/edit/")?>/'+ id,
						dataType: 'json',
						success: function (data) {	
								$('#modal_content').html(data);
			...
			    		keyboard: false
				...
				function remove(id) {
					$.ajax({
						type: 'POST',
						url: '<?= base_url("maintenance/delete/") ?>/' + id,
						dataType: 'json',
						success: function (data) {
							var pageno = $('.paginate_active a').data('ci-pagination-page') - 1;
							load_data();
				...
				$.ajax({
					type: 'POST',
					url: '<?= base_url("maintenance/datagrid/")?>/' + pageno,
					dataType: 'json',
					success: function (data) {
						// console.log(data);
