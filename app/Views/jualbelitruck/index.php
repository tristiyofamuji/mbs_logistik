...
<div id="content" class="app-content">
						...
						<table id="datatableDefault" class="table text-nowrap w-100">
							<thead>
								<tr>
									...
									<th class="border-top-0 pt-0 pb-2">Tgl. Transaksi</th>
									<th class="border-top-0 pt-0 pb-2">File Invoice</th>
		...
		<script type="text/javascript">
				...
				function save(id = "") {
					...
					$.ajax({
						type: 'POST',
						url: '<?= base_url("jualbelitruck/save/") ?>/' + id,
						dataType: 'json',
						data: {
							file_invoice: $('input[name="file_invoice"]').val(),
				...
				$('#toolbar_tambah').on('click', function () {
					$('.main_modal').on('show.bs.modal', function (e) {
						...
						xhr = $.ajax({
							type: 'POST',
							url: '<?= base_url("jualbelitruck/tambah/")?>',
							datatype: 'json',
							success: function (data) {
								setTimeout(function () {
				...
				function edit(id) {
					$('.main_modal').on('show.bs.modal', function (e) {
						...
						xhr = $.ajax({
						type: 'POST',
						url: '<?= base_url("jualbelitruck/edit/")?>/'+ id,
						dataType: 'json',
						success: function (data) {	
								$('#modal_content').html(data);
			...
			    		keyboard: false
				...
				function remove(id) {
					$.ajax({
						type: 'POST',
						url: '<?= base_url("jualbelitruck/delete/") ?>/' + id,
						dataType: 'json',
						success: function (data) {
							var pageno = $('.paginate_active a').data('ci-pagination-page') - 1;
							load_data();
				...
				$.ajax({
					type: 'POST',
					url: '<?= base_url("jualbelitruck/datagrid/")?>/' + pageno,
					dataType: 'json',
					success: function (data) {
						// console.log(data);
